/**
 * Provider-neutral definitions of the "actions" the assistant may propose.
 * Each maps to a tool the LLM can call; the resulting call is surfaced to the
 * user as a proposal and only written to the DB after they approve it.
 */

export type ProposalAction =
  'add_expense' | 'add_income' | 'add_installment' | 'add_debt' | 'add_savings';
...
export const AI_TOOLS: ToolDef[] = [
  {
    name: 'add_expense',
    description:
      'Record money the user spent. Use for purchases, bills, and outgoing payments.',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
        ...
        categoryName: {
          type: 'string',
          description:
            "Category name, matched to the user's existing categories (e.g. Food, Transport). Optional.",
        },
        description: {
          type: 'string',
  ...
  },
  {
    name: 'add_income',
    description:
      'Record money the user received (salary, freelance payment, etc.).',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    description:
      'Record a purchase paid over monthly installments (e.g. a phone on a 6-month plan).',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  },
  {
    name: 'add_debt',
    description:
      'Record a debt or loan. direction OWED_BY_ME = the user owes it; OWED_TO_ME = someone owes the user.',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  },
  {
    name: 'add_savings',
    description:
      'Record a savings asset. For GOLD coins use goldForm (RASHADI_LIRA/SOVEREIGN) with quantity; for gold by weight use JEWELLERY/BAR with weightGrams and karat; for CASH/OTHER use amount.',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
...
/** A short, human-readable one-liner describing a proposed action. */
