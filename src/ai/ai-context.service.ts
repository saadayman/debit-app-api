...
export class AiContextService {
  ...
  /**
   * Builds a complete, user-scoped financial profile for the assistant.
   * Authentication data and the encrypted provider key are intentionally never
   * included. Descriptions and notes are data only and are treated as untrusted
   * text by the assistant's system prompt.
   */
  async build(userId: string): Promise<{
    currency: string;
    locale: string;
    today: string;
    categories: string[];
    ...
    for (const expense of expenses) {
      categorySpending.set(
        ...
        (categorySpending.get(expense.category.name) ?? 0) +
    ...
    const salaryCycles = incomes
      ...
      .map((income) => {
        const linkedExpenses = expenses.filter(
          (expense) => expense.salaryIncomeId === income.id,
        );
        const spent = linkedExpenses.reduce(
    ...
    const data = {
      scope:
        'All financial records available to this authenticated user at chat time.',
      generatedDate: format(now, 'yyyy-MM-dd'),
      preferences: {
        currency,
      ...
      jobs: jobs.map((job) => ({
        ...
        expectedAmount:
          job.expectedAmount == null ? null : amount(job.expectedAmount),
      ...
      debts: debts.map((debt) => {
        ...
        return {
          ...
          monthlyPayment:
            debt.monthlyPayment == null ? null : amount(debt.monthlyPayment),
          totalPaid: paid,
      ...
      installments: installments.map((installment) => {
        const expensePayments = installment.expenses.reduce(
          (sum, expense) => sum + amount(expense.amount),
          0,
        );
        const paidCount =
          installment.paidInstallments + installment.expenses.length;
        const paidAmount = Math.min(
          amount(installment.downPayment) +
            installment.paidInstallments * amount(installment.monthlyAmount) +
            expensePayments,
          amount(installment.totalPrice),
        );
        return {
          id: installment.id,
          ...
          remainingInstallments: Math.max(
            installment.totalInstallments - paidCount,
            0,
          ),
          paidAmount,
          remainingBalance: Math.max(
            amount(installment.totalPrice) - paidAmount,
            0,
          ),
      ...
      budgets: budgets.map((budget) => ({
        id: budget.id,
        month: format(budget.month, 'yyyy-MM'),
      ...
      savings: {
        ...
        assets: savingsAssets.map((asset) => ({
          ...
          name: asset.name,
          weightGrams:
            asset.weightGrams == null ? null : amount(asset.weightGrams),
          karat: asset.karat,
